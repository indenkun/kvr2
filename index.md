...
3.  **Analyze Recommendations**: Explore Kvalseth’s arguments for why
    $R_{1}^{2}$ (and $R_{9}^{2}$ for robust cases) are often preferred
...
- **$R_{1}^{2}$** to **$R_{8}^{2}$**: A classification of existing and
  historical formulas used in statistical literature and software.
- **$R_{9}^{2}$**: A robust version of the coefficient of determination
...
**Observation:** In Case B, notice that $R_{2}^{2}$ and $R_{3}^{2}$
exceed 1.0. This demonstrates why choosing the correct definition is
critical for models without an intercept.
